...
bisnode_raw <- read_csv("https://osf.io/3qyut/download",show_col_types = FALSE) %>% data.table()

#only used this command during work as it was faster
...
```{r,echo=FALSE,fig.align='center',fig.height=3.75,fig.width=6,warning=FALSE,message=FALSE}
h1 <- ggplot(bisnode_final[d2_sales_mil_log>=0,],aes(x = d2_sales_mil_log))+
  geom_histogram()+
  geom_vline(xintercept = log(2),color="red")+
  labs(title = "Distr. of the 2-year\nsales growth",x="",y="")+
  theme_bw()+
...
h2 <- ggplot(bisnode_final[d1_labor_avg_mod_log>=0,],aes(x = d1_labor_avg_mod_log))+
  ...
  labs(title = "Distr. of the 1-year\n(modified) labor growth",x="",y="")+
  theme_bw()+
  theme(axis.text = element_text(size=7,color="black"),
        axis.title = element_text(size=8,color = "black"),
        plot.title = element_text(size =8,hjust =0.5))
h3 <- ggplot(bisnode_final[profit_pl_chg_rm3>=0,],aes(x = profit_pl_chg_rm3))+
  ...
  labs(title = "Distr. of the 3-year\nmoving average\nof profits growth",x="",y="")+
  theme_bw()+
  theme(axis.text = element_text(size=7,color="black"),
        axis.title = element_text(size=8,color = "black"),
        plot.title = element_text(size =8,hjust =0.5))
...
#ggsave(file="histograms.png",h,height = 7,width = 3.5)
```
...
df_cor %>% kable(align = "c")%>% 
  kable_styling(latex_options = "hold_position",
                full_width=FALSE,position = "center",font_size = 5)
```
...
#Final removal of NA-s as they destroy the Logistic part
...
p1 <- createRocPlot(roc_obj_holdout, "best_logit_no_loss_roc_plot_holdout")
p1
#ggsave("AUC.png",p1,height = 6,width = 4)
```
...
For Fold-5, the best threshold is `r round(best_treshold$threshold,3)`, which is close the inverse cost ratio and to the sample frequency of fastgrowths.

```{r,echo=FALSE,fig.align='center',fig.height=3.75,fig.width=6}
p2 <-createLossPlot(roc_obj, best_treshold, "rf_p_loss_plot")
p2
#ggsave("threshold_loss.png",p2,height = 6,width = 5.5)
```
...
```{r,echo=FALSE}
paste0("RMSE on holdout with Model 4: ",
       round(RMSE(data_holdout$best_logit_with_loss_pred,data_holdout$fg_sales_d2),digits = 3))

paste0("RMSE on holdout with Random Forest: ",
       round(RMSE(data_holdout$rf_p_prediction,data_holdout$fg_sales_d2),digits = 3))
```
...
cm_object3 <- confusionMatrix(holdout_prediction,data_holdout$fg_sales_d2_f)
cm3 <- cm_object3$table
cm3 %>%  kable(align = "c",digits = 3, row.names = TRUE) %>%
   kable_styling(latex_options = "hold_position",
                full_width=FALSE,position = "center",font_size = 9)
```
